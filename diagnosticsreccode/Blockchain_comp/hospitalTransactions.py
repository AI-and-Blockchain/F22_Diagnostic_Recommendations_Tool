...
def printPatientStatistics(data_file):
    '''
    Prints the distribution of values of the diabetes patients
    '''
...
def trainModel(data_file, pick_max, pick_other_node_model):
    '''
    Trains the model on a singular hospital node
    '''
...
# FOR FUTURE: upload the model date of current hospital to ipfs.
# (currently handled with transactions since data to transfer is not large)
def uploadModelData(from_hospital, model_data):
    pass

def setUpTransaction(from_hospital, to_hospital, from_pk):
    '''
    Opt-in transaction to ensure hospital nodes can communicate to one another
    '''
...
def sendTransaction(from_hospital, to_hospital, from_pk, cid):
    '''
    Function allowing two hospital nodes to send transactions storing patient or model
    data to one another. Easily adaptable to use IPFS. 
    '''
